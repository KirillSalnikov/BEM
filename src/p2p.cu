...
// ============================================================
// Optimized P2P near-field CUDA kernels (CSR format)
//
// Optimizations:
//   1. Warp-per-target: 32 threads cooperatively process each target.
//      Lanes stripe the source list for coalesced CSR index reads.
//   2. Float32 transcendentals: exp/sincos computed in float (5x faster
//      than double on SM_86), R computed in double for subtraction accuracy,
//      accumulation in double for summation accuracy.
//   3. __ldg() intrinsic for read-only global memory (texture cache path).
//   4. rsqrt() + sincos → single trig eval.
//   5. Block size 128 (4 warps) for register pressure balance.
// ============================================================
    ...
    int Nt)
{
    ...
    const int jstart = __ldg(&offsets[tid]);
    const int jend   = __ldg(&offsets[tid + 1]);

    for (int j = jstart + lane; j < jend; j += WARP_SIZE) {
        int sid = __ldg(&src_idx[j]);
        int sid3 = sid * 3;
...
// ============================================================
// Combined potential + gradient kernel
// ============================================================
...
// ============================================================
// Batch-2 P2P kernels: two charge vectors, single CSR traversal
// Green's function computed once per source-target pair, applied to both charges.
// ============================================================
...
// Batch-2 P2P combined potential + gradient
    ...
    double* __restrict__ gz2_re, double* __restrict__ gz2_im)
{
    ...
    for (int j = jstart + lane; j < jend; j += WARP_SIZE) {
        ...
        if (R2 >= 1e-24) {
            ...
            // Gradient factor (double precision for position-dependent terms)
            double fac_re = (-k_im - inv_R) * inv_R;
            double fac_im = k_re * inv_R;
            double gG_re = G_re * fac_re - G_im * fac_im;
            double gG_im = G_re * fac_im + G_im * fac_re;
