"""
Test the two best PMCHWT sign conventions at ref=1,2,3.
Convention A: sK1=-1, sK2=-1, sB=-1 (best total error from brute force)
Convention B: sK1=+1, sK2=+1, sB=+1 (best Q_sca convergence)

Also test far-field with both sM=+1 and sM=-1 for each.
"""
import numpy as np
import time
...
def compute_Qext_Qsca(rwg, verts, tris, J, M, k, eta, radius, E0, sM=+1):
    ...
    # Q_ext from forward (theta≈0): at theta≈0, phi=0: theta_hat ≈ (1,0,0) = E0
    # So F·E0 ≈ F_theta(0)
...
if __name__ == "__main__":
    ...
    configs = {
        'A: [ηL,K;K,-L/η]·x=b':   (+1, +1, +1),  # Best Q_sca convergence
        'B: [ηL,-K;-K,L/η]·x=-b':  (-1, -1, -1),  # Best total error
        'C: [ηL,K;-K,L/η]·x=b':    (+1, -1, +1),  # Standard textbook PMCHWT
    }
    ...
    for refine in [1, 2, 3]:
        ...
        for name, (sK1, sK2, sB) in configs.items():
            ...
            Q_ext_p, Q_sca_p = compute_Qext_Qsca(rwg, verts, tris, J, M,
                                                    k_ext, eta_ext, radius, E0, sM=+1)
            Q_ext_m, Q_sca_m = compute_Qext_Qsca(rwg, verts, tris, J, M,
                                                    k_ext, eta_ext, radius, E0, sM=-1)

            eE_p = abs(Q_ext_p - Q_ext_mie) / Q_ext_mie * 100
            eS_p = abs(Q_sca_p - Q_sca_mie) / Q_sca_mie * 100
            eE_m = abs(Q_ext_m - Q_ext_mie) / Q_ext_mie * 100
            eS_m = abs(Q_sca_m - Q_sca_mie) / Q_sca_mie * 100

            print(f"  {name}")
            print(f"    sM=+1: Q_ext={Q_ext_p:.4f}({eE_p:5.1f}%) Q_sca={Q_sca_p:.4f}({eS_p:5.1f}%)")
            print(f"    sM=-1: Q_ext={Q_ext_m:.4f}({eE_m:5.1f}%) Q_sca={Q_sca_m:.4f}({eS_m:5.1f}%)")
