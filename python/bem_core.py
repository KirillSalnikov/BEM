...
def build_aext_preconditioner(L_ext, K_ext, eta_ext, N):
    """Build and LU-factorize the exterior-only PMCHWT matrix A_ext.

    A_ext = [[η·L_ext, -K_ext], [K_ext, L_ext/η]]

    Theory: A_ext⁻¹·A = A_ext⁻¹·(A_ext + A_int) = I + A_ext⁻¹·A_int.
    Since A_int is compact, eigenvalues cluster near 1, giving GMRES
    convergence in 7-17 iterations regardless of refractive index.

    Returns (Z_ext_lu, Z_ext) where Z_ext_lu is the LU factorization.
    """
    t0 = _time.time()
    Z_ext = np.zeros((2*N, 2*N), dtype=complex)
    Z_ext[:N, :N] = eta_ext * L_ext
    Z_ext[:N, N:] = -K_ext
    Z_ext[N:, :N] = K_ext
    Z_ext[N:, N:] = L_ext / eta_ext
    Z_ext_lu = linalg.lu_factor(Z_ext)
    print(f"  A_ext preconditioner: LU({2*N}x{2*N}) in {_time.time()-t0:.1f}s")
    return Z_ext_lu, Z_ext


def solve_gmres_aext(Z, b, Z_ext_lu, tol=1e-5, maxiter=200, verbose=True):
    """Solve Z·x = b using GMRES with A_ext LU preconditioner.

    Converges in 7-17 iterations for dielectric PMCHWT.
    For multiple RHS with the same Z, reuse the same Z_ext_lu.
    """
    ...
    def apply_precond(x):
        return linalg.lu_solve(Z_ext_lu, x)
    M = LinearOperator((n, n), matvec=apply_precond, dtype=complex)
    ...
    t0 = _time.time()
    x, info = sp_gmres(Z, b, M=M, rtol=tol, maxiter=maxiter, restart=min(200, n),
                        callback=callback, callback_type='pr_norm')
    elapsed = _time.time() - t0

    if verbose:
        if info == 0:
            print(f"  GMRES(A_ext): {iter_count[0]} iter, {elapsed:.2f}s")
        else:
            res = np.linalg.norm(Z @ x - b) / np.linalg.norm(b)
            print(f"  GMRES(A_ext): no conv (info={info}), {iter_count[0]} iter, res={res:.2e}")

    return x


def solve_gmres(Z, b, tol=1e-4, maxiter=500, precond='block_diag', Z_ext_lu=None):
    """Solve Z·x = b using GMRES with preconditioner.
    ...
    precond: 'aext' uses the exterior-only PMCHWT as preconditioner (best, 7-17 iter),
             'block_diag' uses the 2×2 block-diagonal of Z as preconditioner,
             'diag' uses the main diagonal,
             None for no preconditioning.
    Z_ext_lu: precomputed LU of A_ext (from build_aext_preconditioner), required for 'aext'.
    """
    from scipy.sparse.linalg import LinearOperator
    n = Z.shape[0]
    N = n // 2
    ...
    if precond == 'aext':
        if Z_ext_lu is None:
            raise ValueError("precond='aext' requires Z_ext_lu from build_aext_preconditioner()")
        return solve_gmres_aext(Z, b, Z_ext_lu, tol=tol, maxiter=maxiter)
...
def compute_amplitude_matrix(rwg, verts, tris, k_ext, eta_ext, theta_arr,
                              Z_lu=None, Z=None, Z_ext_lu=None,
    ...
    Parameters:
      ...
      Z_ext_lu : A_ext LU preconditioner (from build_aext_preconditioner).
                 When provided with Z, uses GMRES+A_ext instead of LU.
    ...
    # Determine solve method: GMRES+A_ext or LU
    use_gmres_aext = Z_ext_lu is not None and Z is not None

    if not use_gmres_aext:
        # LU factorization (reusable for multiple RHS)
        if Z_lu is None:
            if Z is None:
                raise ValueError("Provide either Z/Z_lu or Z/Z_ext_lu")
            Z_lu = lu_factor(Z)
    ...
    for pol_name, E0 in [('par', e_par), ('perp', e_perp)]:
        ...
        if use_gmres_aext:
            coeffs = solve_gmres_aext(Z, b, Z_ext_lu)
        else:
...
def compute_mueller_matrix(rwg, verts, tris, k_ext, eta_ext, theta_arr,
    ...
    S = compute_amplitude_matrix(rwg, verts, tris, k_ext, eta_ext, theta_arr,
                                  Z_lu=Z_lu, Z=Z, Z_ext_lu=Z_ext_lu,
                                  k_hat=k_hat, sM=sM,
            ...
            for ig, gamma in enumerate(gamma_nodes):
                ...
                for pol_name, E0 in [('par', e_par_inc), ('perp', e_perp_inc)]:
                    b = compute_rhs_planewave(rwg, verts, tris, k_ext, eta_ext,
                                               E0=E0, k_hat=k_hat, quad_order=quad_order)
                    if use_gmres_aext:
                        coeffs = solve_gmres_aext(Z, b, Z_ext_lu, verbose=False)
                    else:
                        coeffs = lu_solve(Z_lu, b)
...
def orientation_average_mueller_batched(rwg, verts, tris, k_ext, eta_ext, theta_arr,
    ...
    If Z_ext_lu is provided (from build_aext_preconditioner), uses GMRES+A_ext
    preconditioner instead of LU. This avoids the O(N³) LU factorization of Z
    and solves each RHS in 7-17 GMRES iterations. Useful when N is large and
    LU of Z is too expensive or out of memory.
    ...
    # --- Phase 2: Solve all RHS ---
    if use_gmres_aext:
        print(f"    GMRES(A_ext): solving {total*2} RHS columns...")
        X = np.zeros_like(B)
        for c in range(total * 2):
            if (c + 1) % 20 == 0 or c == 0:
                print(f"      GMRES column {c+1}/{total*2}...", flush=True)
            X[:, c] = solve_gmres_aext(Z, B[:, c], Z_ext_lu, verbose=False)
        print(f"    GMRES(A_ext): all {total*2} RHS solved.")
    else:
        print(f"    Batched LU: solving {total*2} RHS at once...")
        X = lu_solve(Z_lu, B)
...
def orientation_average_mueller_adaptive(rwg, verts, tris, k_ext, eta_ext, theta_arr,
    ...
    Parameters:
      ...
      batch_size    : orientations per batch (for batched lu_solve or GMRES+A_ext)
